...
public class Q5_TrajAndSpeedFence {
    ...
    public static DataStream<TrajSpeedWindowFn.TrajSpeedOut> build(StreamExecutionEnvironment env, DataStream<GpsEvent> events,
        ...
        // Convert PreparedGeometry fence to GeoFlink Polygons
        List<Polygon> fencePolygons = new ArrayList<>();
        for (PreparedGeometry pg : fenceBuffered) {
            ...
            if (geom instanceof org.locationtech.jts.geom.Polygon) {
                ...
                fencePolygons.add(geoPolygon);
            }
        }

        // Filter points that are within the fence
        DataStream<Point> inFence = pointStream.filter(new FilterFunction<Point>() {
            @Override
            public boolean filter(Point point) throws Exception {
                // Check if point is in any fence polygon
                for (Polygon fencePoly : fencePolygons) {
                    if (fencePoly.polygon.intersects(point.point)) {
                        return true; // Point is in fence, include it
                    }
                }
                return false; // Point is not in any fence, exclude it
            }
        });

        // Convert back to EnrichedEvent format for trajectory and speed calculation
        DataStream<EnrichedEvent> enrichedStream = inFence.map(new MapFunction<Point, EnrichedEvent>() {
            @Override
        ...
        return enrichedStream
                ...
                .filter(o -> o.avgSpeed > avgThreshold || o.minSpeed > minThreshold);
    }
}
